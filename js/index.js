;(function() {
  ...
  var pTagList = document.querySelectorAll('p img')

  for (var i = 0; i < pTagList.length; i++) {
    pTagList[i].style.width = '100%'
    pTagList[i].style.objectFit = 'cover'
    pTagList[i].style.objectPosition = 'center'
    pTagList[i].parentNode.style.textAlign = 'center'
  }

  var preTagList = document.querySelectorAll('pre')

  for (var i = 0; i < preTagList.length; i++) {
    console.log(preTagList[i].parentElement.clientWidth)
    preTagList[i].style.width = `${preTagList[0].parentElement.width}px`
  }
